<?php
    ...
    $sql = "SELECT DISTINCT l.authority, m.profile, l.id, l.password, m.name, m.call_number FROM login AS l, member AS m, student AS s WHERE l.id = m.id AND l.id = s.id AND s.course_code IN (SELECT course_code FROM teacher WHERE id = '$id')";
